...
void PlayersManager::Play_note(uint8_t instrument_id, uint8_t note_number, float velocity_float, int track) // after receiving a NoteOn command
{
    ...
    if (finished)
    {
        ...
        Player_ptr[id_player].Set_ADSR(Preset[instrument_id].attack, Preset[instrument_id].decay, Preset[instrument_id].sustain, Preset[instrument_id].release, Preset[instrument_id].attack_type);
...
void PlayersManager::Release_all_players_for_instrument_solo(int instrument_id)
{
    for (auto player = 0; player < PLAYERS; ++player)
    {
        ...
        // Map_one_Instrument_for_all_notes(instrument_id); // E' PALESEMENTE UN ERRORE!!!
    }
}
