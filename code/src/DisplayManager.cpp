...
FLASHMEM
void DisplayManager::Show_Patch_number(bool change_patch)
{
    tft.setCursor(x_pos(30), y_pos(0));
    tft.setTextColor(TEXT_COLOR);
    tft.print("PATCH");
    tft.setCursor(x_pos(35.5), y_pos(0));
    tft.setTextColor(change_patch ? ILI9341_YELLOW : ILI9341_WHITE);
    tft.print(Patch_id);
}

FLASHMEM
void DisplayManager::Patch_volume(bool change_vol)
{
    tft.setCursor(x_pos(41), y_pos(0));
    tft.setTextColor(TEXT_COLOR);
    tft.print("VOLUME");
    Patch_volume_value(change_vol);
}

FLASHMEM
void DisplayManager::Patch_volume_value(bool change_vol)
{
...
void DisplayManager::Led_PERFORMANCE_instrument(int instrument_id, bool on)
{
    if (on)
    {
        tft.drawBitmap(X_LED, Instrument_Y_POSITION(position_of_Instrument[instrument_id]), led_pic, 8, 8, (MX_mute[instrument_id] ? RED_ON : GREEN_ON));
    }
    else
    {
        tft.drawBitmap(X_LED, Instrument_Y_POSITION(position_of_Instrument[instrument_id]), led_pic, 8, 8, (MX_mute[instrument_id] ? RED_OFF : GREEN_OFF));
    }
}
...
void DisplayManager::Led_SOUND_EDIT_instrument(int instrument_id, bool on)
{
    if (on)
    {
        tft.drawBitmap(x_pos(22) - 4, y_pos(0), led_pic, 6, 8, (MX_mute[instrument_id] ? RED_ON : GREEN_ON));
    }
    else
    {
        tft.drawBitmap(x_pos(22) - 4, y_pos(0), led_pic, 8, 8, (MX_mute[instrument_id] ? RED_OFF : GREEN_OFF));
    }
    return;
}
...
void DisplayManager::Led_INSTRUMENT_VCF_instrument(int instrument_id, bool on)
{
    ...
    else if (Lilla_state_0 == LIVE_SAMPLING)
    {
        if (on)
        {
            tft.drawBitmap(x_pos(40) - 4, y_pos(0), led_pic, 6, 8, (MX_mute[instrument_id] ? RED_ON : GREEN_ON));
        }
        else
        {
            tft.drawBitmap(x_pos(40) - 4, y_pos(0), led_pic, 8, 8, (MX_mute[instrument_id] ? RED_OFF : GREEN_OFF));
        }
    }
}
...
void DisplayManager::Led_LIVE_SAMPLING(bool on)
{
    if (on)
    {
...
void DisplayManager::Led_tuning_tone(int patch_id)
{
    ...
    {
        tft.drawBitmap(X_LED, Instrument_Y_POSITION(Patch[patch_id].instruments), led_pic, 8, 8, (TT_playing ? ILI9341_RED : RED_OFF));
    }
}

FLASHMEM
void DisplayManager::Performance_page(bool change_patch, bool change_vol)
{
    instrument_editing_flag = false;
    Patch_header(change_patch, change_vol);
    Performance_menu(); // displays the menu and updates P_menu_max
    Instruments_header();
    All_Instrument(Patch_id);
}
...
void DisplayManager::LOOP_page(void)
{
    ...
    for (auto instrument_id = 0; instrument_id < INSTRUMENTS_MAX; ++instrument_id)
    {
        ...
        {
            tft.setCursor(x_pos(Loop_HEAD_C + 3), 151 + instrument_id * 11);
            tft.print(instrument_id + 1);
        }
    }
...
void DisplayManager::Frame_loop_menu(int position, bool fresh) // after Loop_menu() use fresh = true
{
    ...
    {
        Frame(Loop_X_position_menu_0, 1, Loop_dimension_voice_menu_0, false); // Frame(X_position_Menu_Loop[position_0], 1, dimension_voice_Menu_Loop[element_Menu_Loop[position_0]], false);
    }
...
void DisplayManager::Delete_all_frame_loop_menu(void)
{
    for (auto element = 0; element < 4; ++element)
    {
        if (Menu_Loop[element])
        {
            Frame(X_position_Menu_Loop[position_Menu_Loop[element]], 1, dimension_voice_Menu_Loop[element], false);
        }
    }
}
...
{
    Frame(X_position_Menu_DS[DS_frame_menu_position_0], Y_position_Menu_DS[DS_frame_menu_position_0], dimension_voice_Menu_DS[element_Menu_DS[DS_frame_menu_position_0]], false);
...
FLASHMEM
void DisplayManager::Frame_LS_menu(int position)
{
...
FLASHMEM
void DisplayManager::Patch_header(bool change_patch, bool change_vol)
{
    tft.fillScreen(ILI9341_BLACK);
    Show_Performance();
    Show_Patch_number(change_patch);
    Patch_volume(change_vol);
    Show_effects();
}
...
FLASHMEM
void DisplayManager::Show_Instrument_description(int patch_id, int instrument_id, bool editing)
{
    auto position = position_of_Instrument[instrument_id];
...
FLASHMEM
void DisplayManager::All_Instrument(int patch_id)
{
    ...
    for (auto instrument_id = 0; instrument_id < INSTRUMENTS_MAX; ++instrument_id)
    {
        if (Patch[patch_id].Instrument[instrument_id].used)
        {
            Show_Instrument_description(patch_id, instrument_id, false);
        }
    }

    if (tuning_tone_flag)
    {
        Instrument_TT(patch_id);
    }
}

FLASHMEM
void DisplayManager::Instrument_TT(int patch_id)
{
...
FLASHMEM
void DisplayManager::Volume_TT(int patch_id)
{
    auto position = Patch[patch_id].instruments;
...
FLASHMEM
void DisplayManager::Frame_performance_menu(int position, bool fresh)
{
    if (!fresh)
    {
        Frame(X_position_Menu_P[Performance_menu_frame_position_0], Y_position_Menu_P[Performance_menu_frame_position_0], dimension_voice_Menu_P[element_Menu_P[Performance_menu_frame_position_0]], false);
    }
    ...
    Performance_menu_frame_position_0 = position;
}
...
FLASHMEM
void DisplayManager::Confirm_patch_change_popup(void)
{
    L_POPUP = 106;
    H_POPUP = 47;
    X_POPUP = (320 - L_POPUP) / 2;
...
FLASHMEM
void DisplayManager::Confirm_patch_change_popup_frame(int value)
{
...
void DisplayManager::Confirm_frame(int X, int Y, int chars, bool print)
{
...
FLASHMEM
void DisplayManager::Confirm_patch_delete_popup(void)
{
    L_POPUP = 106;
    H_POPUP = 47;
    Y_POPUP = (240 - H_POPUP) / 2;
...
FLASHMEM
void DisplayManager::Confirm_patch_delete_popup_frame(int value)
{
...
void DisplayManager::Encoder_pushbutton_test_board(void)
{
    ...
    tft.setCursor(X_POPUP + x_pos(1), Y_POPUP + 4);
    tft.print("ENCODERS AND PUSHBUTTONS TEST");
}
...
void DisplayManager::Encoder_pushbutton_test_result(const int device, const int element, const int value)
{
    ...
    for (auto i = 0; i < 5; ++i)
    {
        tft.setCursor(X_POPUP + x_pos(1), Y_POPUP + y_pos(5 - i));
        if (memo[i].device == 1)
        {
            tft.print("encoder ");
...
void DisplayManager::Instrument_VCF_page(int patch_id, int instrument_id)
{
    ...
    else
    {
        ...
        tft.setTextColor(TEXT_COLOR);
        tft.print("VOLUME");
        Patch_volume_value(true); // true: YELLOW
    }
...
void DisplayManager::Show_LS_ring_tape_wave(int sound_id)
{
    ...
    for (auto wave_pixel = 0; wave_pixel < WAVE_WIDTH; ++wave_pixel)
    {
        yp = CANVAS_WAVE_0 - (*(X + wave_pixel) >> 10);
        yn = CANVAS_WAVE_0 - (*(X + wave_pixel + WAVE_WIDTH) >> 10);

        uint16_t LS_wave_color = Get_LS_wave_color(wave_pixel);
...
void DisplayManager::Show_wave(int instrument_id)
{
    ...
    NC_A = ((WAVE_WIDTH * (Preset[instrument_id].Noclick < Noclick_max ? Preset[instrument_id].Noclick : Noclick_max)) / (Preset[instrument_id].B - Preset[instrument_id].A));
    ...
    for (auto wave_pixel = 0; wave_pixel < WAVE_WIDTH; ++wave_pixel)
    {
        ...
        yn = CANVAS_WAVE_0 - (*(X + wave_pixel + WAVE_WIDTH) >> 10) * volume_float;
