...
void StereoLiveSampler::update(void)
{
    ...
    {
        memcpy((uint32_t *)(_LS_buffer_L + Q_sample + 1), (uint32_t *)in_block_L->data, 256); // memcpy(destination pointer, origin pointer, bytes to be copied)
        memcpy((uint32_t *)(_LS_buffer_R + Q_sample + 1), (uint32_t *)in_block_R->data, 256); // memcpy(destination pointer, origin pointer, bytes to be copied)
    }
    else // si copiano i sample entranti su due Buffer, poi il valore medio viene copiato su un unico array mono
    {
        ...
        for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++)
            _LS_buffer_mono[Q_sample + 1 + i] = (Buffer_L[i] >> 1) + (Buffer_R[i] >> 1);
    }
    ...
    if(cancel_beyond)
    {
        ...
        if (stereo)
        {
            ...
            {
              *(uint32_t *)(_LS_buffer_L + i) = 0;
              *(uint32_t *)(_LS_buffer_R + i) = 0;
            }
        }
        else
        {
    ...
    if (attack_gain_flag)
    {
        if (stereo)
            for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++)
            {
                ...
                _LS_buffer_L[Q_sample_cache + 1 + i] = _LS_buffer_L[Q_sample_cache + 1 + i] * factor;
                _LS_buffer_R[Q_sample_cache + 1 + i] = _LS_buffer_R[Q_sample_cache + 1 + i] * factor;
                samples_counter--;
            }
            ...
            {
                float factor = (1.0 - samples_counter / Slope_samples);
                _LS_buffer_mono[Q_sample_cache + 1 + i] = _LS_buffer_mono[Q_sample_cache + 1 + i] * factor;
                samples_counter--;
            }
