    ...
    if (run_flag)
    {
        if (flag[INSTRUMENT_ROUTE])
        {
            ...
            for (int i = 0; i < INSTRUMENTS_MAX; ++i)
            {
                ...
                _Players_Manager->MX_multicast_change_routing(Delay_values.instrument_route[i]);
            }
        ...
        if (flag[MODULATION_SOURCE])
        {
            ...
            _Delay_L->Set_delay_modulation_source(Delay_values.modulation_source); // Left channel
            _Delay_R->Set_delay_modulation_source(Delay_values.modulation_source); // Right channel
        ...
        if (flag[SAMPLES])
        {
            ...
            if (Delay_values.samples_LR >= 0)
                _Delay_L->Set_delay_central_value(Delay_values.samples + Delay_values.samples_LR); // Left channel
            else
                _Delay_R->Set_delay_central_value(Delay_values.samples - Delay_values.samples_LR); // Left channel
        ...
        if (flag[MODULATION_DEPTH])
        {
            ...
            // trasmetti nuovo valore
            _Delay_L->Set_delay_modulation_gain(Delay_values.modulation_depth);
            _Delay_R->Set_delay_modulation_gain(Delay_values.modulation_depth);
        ...
        if (flag[MODULATION_FREQUENCY])
        {
            ...
            // trasmetti nuovo valore
            _LFO_D[0]->Set_frequency(Delay_values.modulation_frequency);
            _LFO_D[1]->Set_frequency(Delay_values.modulation_frequency);
        ...
        if (flag[MODULATION_PHASE_LR])
        {
            ...
            // trasmetti nuovo valore
            _LFO_D[0]->Set_phase(Delay_values.modulation_phase_LR);
