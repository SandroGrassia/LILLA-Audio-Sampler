        ...
        if (MIDI.read())
        {
            ...
            case midi::NoteOn:
            {
                ...
                if (tuning_tone_flag)
                {
                    ...
                    _Tone_generator->Frequency(pitch_from_note[note_number] * 261.63);
                    _Tone_generator->Amplitude(Volume_float[tuning_tone_volume]);
                    _Tone_generator->Start();
                    TT_playing = true;
                    TT_led_flag = true;
                }
                ...
                _Players_Manager->Reset_booked_and_restart_player();

                _Players_Manager->Reset_players_to_restart();
                ...
                for (uint8_t I = 0; I < INSTRUMENTS_MAX; ++I)
                    if ((Session[session].Instrument[I].used) && bitRead(map_instrument_for_note[midi_channel][note_number], I))
                        _Players_Manager->Play_note(I, note_number, velocity_float, -1);
                ...
                if (_Players_Manager->Get_players_to_restart() > 0)
                    _Players_Manager->Calculate_and_set_mix_samples();
            ...
            case midi::NoteOff:
            {
                ...
                _Players_Manager->Multicast_stop_players_for_NoteOff(midi_channel, note_number, -1);
            ...
            case midi::PitchBend:
            {
                ...
                _Players_Manager->Broadcast_pitch_bend(midi_channel, pitch_bend_value[midi_channel]);
            ...
            case midi::AfterTouchChannel: // Status = 1101nnnn Pressure value = 0vvvvvvv
            {
                ...
                for (int I = 0; I < INSTRUMENTS_MAX; I++)
                    ...
                    {
                        _Players_Manager->Multicast_IF_index(I, Preset[I].Filter.index * after_touch_channel_value[midi_channel]);
                    }
                if (Lilla_state == MIDI_MONITOR && !display_wait)
                {
            ...
            case midi::ControlChange: // Status = 1011nnnn  Controller = 0ccccccc (da 0 a 120)  Value = 0vvvvvvv
            {
                ...
                if (controller == 123) // All sound off
                {
                    ...
                    _Players_Manager->Multicast_all_notes_off(midi_channel);
                }
                ...
                else if (controller == 1) // Modulation
                {
                    ...
                    if (midi_value > 0)
                    {
                        _Vibrato->Update_vibrato_array(midi_value);
                        if (!vibrato_flag)
                        {
                            vibrato_flag = true;
                            _Players_Manager->Multicast_update_vibrato(midi_channel, vibrato_flag);
                        }
                    }
                    else
                    {
                    ...
                    for (int instrument = 0; instrument < INSTRUMENTS_MAX; ++instrument)
                    {
                        if ((controller == CC_Sound_gain[instrument]) && (controller > 0))
                        {
                            if (Session[session].Instrument[instrument].used && (Get_midi_channel(session, instrument) == midi_channel))
                            {
                                ...
                                _Players_Manager->Update_Preset_volume(session, instrument, Volume_float[volume_session]);
                                _Players_Manager->Multicast_volume_for_instrument_edit(instrument);
    ...
    if (Lilla_state == MIDI_LOOP || (Lilla_state_0 == MIDI_LOOP && (Lilla_state == DELAY_SETTINGS || Lilla_state == SOUND_EDIT || Lilla_state == INSTRUMENT_VCF || Lilla_state == INSTRUMENT_VCF || Lilla_state == MIXER)))
    {
        ...
        if (LOOP_metronomo_run && (LOOP_Clock_frozen >= LOOP_metronomo.metro_time))
        {
            if (Lilla_state == MIDI_LOOP)
                LOOP_metronomo_flag_IN[1] = true; // chiede a track() di aggiornare il metronomo
        ...
        for (int track = 0; track < TRACKS; track++)
        {
            if (LOOP_track_run[track] && (LOOP_Clock_frozen >= LOOP_play_time[track]))
            {
                ...
                if (event == 0)
                {
                    _Players_Manager->Multicast_stop_players_for_loop_track(track);
                }
                ...
                if (LOOP_element[track][event].note_on)
                {
                    ...
                    for (uint8_t I = 0; I < INSTRUMENTS_MAX; I++)
                        if ((Session[session].Instrument[I].used) && bitRead(map_instrument_for_note[midi_channel][note_number], I))
                            _Players_Manager->Play_note(I, note_number, velocity_float, track);
                ...
                else if (!LOOP_element[track][event].note_on)
                {
                    ...
                    _Players_Manager->Multicast_stop_players_for_NoteOff(midi_channel, note_number, track);
                }
