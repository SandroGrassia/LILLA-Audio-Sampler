...
void StereoDelay::update(void)
{
    ...
    {
        _LFO->Update();                                                  // 1: periodic (sinus)
        delay_modulation = delay_modulation_gain_value * _LFO->block[0]; // read only the first value
    ...
    {
        if (!in_block)
            *(DELAY_fifo + sample_write) = 0; // delay_Main_Array[sample_write] = 0;
        else
            *(DELAY_fifo + sample_write) = in_block->data[i]; // delay_Main_Array[sample_write] = in_block->data[i];
