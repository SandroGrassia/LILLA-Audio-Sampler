...
void ArchivingManager::Save_CC_lowpass_filter(uint8_t CC_lowpass_filter)
{
...
void ArchivingManager::Save_CC_Sound_gain(uint8_t instrument, uint8_t CC_Sg_instrument)
{
    Eeprom_writeAnything(LOCATION_CC_SETTINGS + instrument, CC_Sg_instrument);
}

void ArchivingManager::Read_CC_Sound_gain(uint8_t instrument, uint8_t &CC_Sg_instrument)
{
...
void ArchivingManager::Save_Sound(uint8_t sound_id)
{
    Eeprom_writeAnything(Get_location_of_Sound(sound_id), Sound[sound_id]);
}

void ArchivingManager::Read_Sound(uint8_t sound_id, Sound_struct &Sound_this_sound_id)
{
    Eeprom_readAnything(Get_location_of_Sound(sound_id), Sound_this_sound_id);
}

void ArchivingManager::Save_Patch(uint8_t patch_id, Patch_struct Patch_patch)
{
    ...
    {
        EEPROM_Patch.Instrument[instrument].used = Patch_patch.Instrument[instrument].used;
        EEPROM_Patch.Instrument[instrument].sound_id = Patch_patch.Instrument[instrument].sound_id;
        EEPROM_Patch.Instrument[instrument].root_key = Patch_patch.Instrument[instrument].root_key;

        EEPROM_Patch.Instrument[instrument].from_note = Patch_patch.Instrument[instrument].from_note;
        EEPROM_Patch.Instrument[instrument].to_note = Patch_patch.Instrument[instrument].to_note;
        EEPROM_Patch.Instrument[instrument].info = (Patch_patch.Instrument[instrument].precedence == true ? 0b1 : 0b0) + (Patch_patch.Instrument[instrument].lock == true ? 0b10 : 0b00); // bit0: precedence, bit1: lock

        EEPROM_Patch.Instrument[instrument].Filter.data = Patch_patch.Instrument[instrument].Filter.use + (Patch_patch.Instrument[instrument].Filter.modulation << 1) + (Patch_patch.Instrument[instrument].Filter.type << 4); // bit0: use  bit1,2,3: modulation  bit4,5: type
        EEPROM_Patch.Instrument[instrument].Filter.pivot = Patch_patch.Instrument[instrument].Filter.pivot;                                                                                                                    // 0 --> 100 filter frequency/note frequency
        EEPROM_Patch.Instrument[instrument].Filter.resonance = Patch_patch.Instrument[instrument].Filter.resonance;                                                                                                            // 0 --> 40
        EEPROM_Patch.Instrument[instrument].Filter.index = Patch_patch.Instrument[instrument].Filter.index;                                                                                                                    // 1 --> 20 modulation_index
        EEPROM_Patch.Instrument[instrument].Filter.frequency_time = Patch_patch.Instrument[instrument].Filter.frequency_time;                                                                                                  // 0 --> 20
    }

    Eeprom_writeAnything(GET_location_of_Patch(patch_id), EEPROM_Patch);
}

void ArchivingManager::Read_Patch(uint8_t patch_id)
{
    ...
    for (auto instrument = 0; instrument < INSTRUMENTS_MAX; ++instrument)
    {
        Patch[patch_id].Instrument[instrument].used = EEPROM_Patch.Instrument[instrument].used;
        Patch[patch_id].Instrument[instrument].sound_id = EEPROM_Patch.Instrument[instrument].sound_id;

        // Serial.println("Patch[patch_id].Instrument[instrument].sound_id"); Serial.println(EEPROM_Patch.Instrument[instrument].sound_id);

        Patch[patch_id].Instrument[instrument].root_key = EEPROM_Patch.Instrument[instrument].root_key;

        Patch[patch_id].Instrument[instrument].from_note = EEPROM_Patch.Instrument[instrument].from_note;
        Patch[patch_id].Instrument[instrument].to_note = EEPROM_Patch.Instrument[instrument].to_note;
        Patch[patch_id].Instrument[instrument].precedence = bitRead(EEPROM_Patch.Instrument[instrument].info, 0);
        Patch[patch_id].Instrument[instrument].lock = bitRead(EEPROM_Patch.Instrument[instrument].info, 1);

        Patch[patch_id].Instrument[instrument].Filter.use = bitRead(EEPROM_Patch.Instrument[instrument].Filter.data, 0);                                                                                                                                            // yes/no
        Patch[patch_id].Instrument[instrument].Filter.type = bitRead(EEPROM_Patch.Instrument[instrument].Filter.data, 4) + 2 * bitRead(EEPROM_Patch.Instrument[instrument].Filter.data, 5);                                                                         // bit4,5:filter_type
        Patch[patch_id].Instrument[instrument].Filter.pivot = EEPROM_Patch.Instrument[instrument].Filter.pivot;                                                                                                                                                     // 0 --> 100 filter frequency/note frequency
        Patch[patch_id].Instrument[instrument].Filter.resonance = EEPROM_Patch.Instrument[instrument].Filter.resonance;                                                                                                                                             // 0 --> 40
        Patch[patch_id].Instrument[instrument].Filter.modulation = bitRead(EEPROM_Patch.Instrument[instrument].Filter.data, 1) + 2 * bitRead(EEPROM_Patch.Instrument[instrument].Filter.data, 2) + 4 * bitRead(EEPROM_Patch.Instrument[instrument].Filter.data, 3); // bit1,2,3:modulation
        Patch[patch_id].Instrument[instrument].Filter.index = EEPROM_Patch.Instrument[instrument].Filter.index;                                                                                                                                                     // 1 --> 20 modulation_index
        Patch[patch_id].Instrument[instrument].Filter.frequency_time = EEPROM_Patch.Instrument[instrument].Filter.frequency_time;                                                                                                                                   // 0 --> 20
    }
}
...
template <class T>
int ArchivingManager::Eeprom_writeAnything(int address, const T &source)
{
    if (address >= 0)
    {
...
template <class T>
int ArchivingManager::Eeprom_readAnything(int address, T &destination)
{
    if (address >= 0)
    {
...
void ArchivingManager::Delete_Patch_Delay_data_in_SD(int patch_id)
{
...
void ArchivingManager::Copy_Patch_from_RAM_to_SD(int patch_id) // public
{
    ...
    if (SD.begin(BUILTIN_SDCARD))
    {
        ...
        if (file)
        {
            Copy_Patch_from_RAM_to_File(patch_id, file);
            file.close();

            Serial.print(F("ArchivingManager::Copy_Patch_from_RAM_to_SD - Patch_data saved in "));
            Serial.print(full_path_ptr);
            Serial.println(" in SD.");
        }
    }
...
void ArchivingManager::Copy_Patch_from_RAM_to_File(int patch_id, File &file) // private
{
    const byte *data = (const byte *)(const void *)&Patch[patch_id];
...
void ArchivingManager::Copy_Sound_from_RAM_to_SD(int patch_id, int instrument_id) // public
{
    String filename = Filename_Sound(patch_id, instrument_id);
    String full_path = String("/LILLASOUND/" + filename);
    auto *full_path_ptr = &full_path[0];
    ...
    if (SD.begin(BUILTIN_SDCARD))
    {
        ...
        Serial.print(F("ArchivingManager::Copy_Sound_from_RAM_to_SD - Patch_data will be saved in "));
        Serial.print(full_path);
        Serial.println(" in SD.");
        ...
        if (file)
        {
            Copy_Sound_from_RAM_to_File(patch_id, instrument_id, file);
            file.close();
        }
    }
    else
...
void ArchivingManager::Copy_Sound_from_RAM_to_File(int patch_id, int instrument_id, File &file) // private
{
    const byte *data = (const byte *)(const void *)&Sound[Patch[patch_id].Instrument[instrument_id].sound_id];
...
bool ArchivingManager::Copy_Patch_from_SD_to_RAM(int patch_id) // public
{
    ...
    if (SD.begin(BUILTIN_SDCARD))
    {
        if (SD.exists(full_path_ptr))
        {
            Serial.print(F("ArchivingManager::Copy_Patch_from_SD_to_Eeprom - delay data file "));
            Serial.print(full_path);
            Serial.println(F(" found; now starts data import."));
            ...
            if (file)
            {
                Copy_Patch_from_SD_to_RAM(file);
                file.close();
                return true;
            }
            else
        ...
        {
            Serial.println(F("ArchivingManager::Copy_Patch_from_SD_to_RAM - delay data not found on SD!"));
            return false;
        }
    }
    else
...
void ArchivingManager::Copy_Sound_from_SD_to_RAM(int patch_id, int instrument_id, int sound_id, File &file) // private
{
    ...
    */
    ...
    if (true)
    {
        Serial.println(F("ArchivingManager::Copy_Sound_from_SD_to_RAM(int patch_id, int instrument_id, File &file) - Done."));
    }
}