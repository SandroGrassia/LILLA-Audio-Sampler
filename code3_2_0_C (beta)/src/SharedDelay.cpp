...
void Print_Delay_values(Delay_values_struct Delay_values)
{
    ...
    for (auto instrument = 0; instrument < INSTRUMENTS_MAX; ++instrument)
    {
        ...
        Serial.print("]: ");
        Serial.println(Delay_values.instrument_route[instrument]);
    }
    Serial.println();
}
...
void Calc_delay_routing(uint8_t value)
{
    for (auto instrument = 0; instrument < INSTRUMENTS_MAX; ++instrument)
    {
        Delay_values.instrument_route[instrument] = (bitRead(value, instrument) == 1 ? true : false);
...
void Turn_ON_Delay(bool ON) // switch on/off Delay (using Instrument routing)
{
    ...
    if (!ON)
    {
        for (auto instrument = 0; instrument < INSTRUMENTS_MAX; ++instrument)
        {
            instrument_route[instrument] = Delay_values.instrument_route[instrument];
            Delay_values.instrument_route[instrument] = 0;
        }
    }
    ...
    else
    {
        for (auto instrument = 0; instrument < INSTRUMENTS_MAX; ++instrument)
        {
            Delay_values.instrument_route[instrument] = instrument_route[instrument];
        }
    }
}
