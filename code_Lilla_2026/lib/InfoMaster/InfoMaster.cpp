...
{
    for (auto i = 0; i < 2 * DisplayPrimitives::WAVEBOARD_WIDTH; ++i)
    {
        samples_620_array[i] = 0;
    }

    // (B - A + 1) >= 100
    float samples_per_pixel = (B - A + 1) / DisplayPrimitives::WAVEBOARD_WIDTH_F;                                     // fondamentale il "." nel divisore!!
    ...
    for (auto i = 0; i < DisplayPrimitives::WAVEBOARD_WIDTH; ++i)
    {
        ...
        samples_620_array[i + DisplayPrimitives::WAVEBOARD_WIDTH] = min_neg;
    }
...
int16_t *InfoMaster::LS_620_samples_array(int file_id, int A_window_sample, int B_window_sample)
{
    ...
    */
    ...
    samples_per_line = ceil((B_window_sample - A_window_sample + 1) / DisplayPrimitives::WAVEBOARD_WIDTH_F);
