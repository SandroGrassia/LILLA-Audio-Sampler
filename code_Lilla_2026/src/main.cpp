...
/*
    ...
    - SPI Display_old ILI9341 240x320
...
GFXcanvas16 canvas = GFXcanvas16(DisplayPrimitives::WAVEBOARD_WIDTH, DisplayPrimitives::WAVEBOARD_HEIGHT); // https://github.com/adafruit/Adafruit-GFX-Library/blob/master/Adafruit_GFX.cpp ; GFXcanvas16 creates an array of w*h*2 bytes in memory
...
// >>>>>>> PERFORMANCE
// Pointer
constexpr int P_field_description_max = 100; // field = where is the pointer
constexpr int P_Instrument_inside_elements = 8;
int P_pointer = 0; // field (position) of the pointer: 0 <= P_pointer <= P_pointer_max
int P_pointer_old = 0;
P_field_description_struct P_field_description[P_field_description_max];
void P_Update_field_description(void);                                // also updates P_pointer_max
void P_Restore_pointer(int old_menu_elements, int new_menu_elements); // must be called when P_menu_elements changes
P_pointer_position_struct P_pointer_position;
...
int P_instrument_id_from_pointer(const int pointer);
int P_sound_id_from_pointer(const int pointer);
int P_sound_id_from_instrument_id(const int instrument_id);
void P_print_pointer_and_field_desciption(const int pointer);
void P_set_pointer_to_last_instrument(const int instrument_id);
void P_set_pointer_to_patch(void);
...
uint8_t S_Clone_Instrument(const int instrument_id); // insert ONE new instrument BELOW instrument
...
constexpr int EN_PB_LieOutVol = 17;
...
void setup()
{
    ...
    */
    ...
    if (Read_pushbutton(7)) // TUNING TONE
    {
        ...
        Display_old.Factory_reset_wait_popup();
    ...
    {
        Display_old.Encoder_pushbutton_test_board();
        ...
        while (true)
        {
            ...
            for (auto i = 0; i < ENCODERS; ++i)
            {
                ...
                if (R != 0)
                {
                    Display_old.Encoder_pushbutton_test_result(1, i, R);
            ...
            for (auto i = 0; i < PUSHBUTTONS; ++i)
            {
                ...
                if (R == true)
                {
                    Display_old.Encoder_pushbutton_test_result(2, i, 0);
...
#pragma region Area_Comune [rgba(118,110,2,0.1)]
    ...
    */
    ...
    if (Read_pushbutton(7)) // switch ON/OFF the Tuning Tone
    {
        ...
        {
            Display_old.P_show_TuningTone_instrument(Patch_id);
        }
        if (!tuning_tone_flag)
        {
            Tone_generator.Stop();
    ...
    if (tuning_tone_flag && Read_encoder(7, tuning_tone_volume, 40, 0, 1))
    {
        if (Lilla_state == PERFORMANCE)
        {
            Display_old.P_show_gain_TT(Patch_id);
        }
    }
...
#pragma region Performance [rgba(2, 68, 118, 0.1)]
    ...
    if (Lilla_state == PERFORMANCE)
    {
        ...
        // Change volume_patch
        if (Read_encoder(17, volume_patch, 40, 0, 1)) // LINE OUT VOLUME
        {
            AudioNoInterrupts();
            ...
            Display_old.P_Patch_volume_value(true);
        }

        // Change P_pointer using encoder
        result = Read_encoder_simple(EN_PB_Select);
        if (result != 0)
        {
            changed = false;
            P_pointer_old = P_pointer;

            switch (P_field_description[P_pointer].field_name)
            {
            case field_Menu:
                if (result == +1)
                {
            ...
            case field_Instrument:
                if (result == +1)
                {
                    if (P_field_description[P_pointer].instrument_line < (Patch[Patch_id].instruments - 1))
                    {
                        changed = true;
                        P_pointer += P_Instrument_inside_elements + 1;
                    }
                }
                else if (result == -1)
                {
                    if (P_field_description[P_pointer].instrument_line == 0)
                    {
                        changed = true;
                        --P_pointer;
                    }
            ...
            case field_Instrument_inside:
                ...
                if (result == +1)
                {
                    if (P_field_description[P_pointer].element < (P_Instrument_inside_elements - 1))
                    {
                        changed = true;
                        ++P_pointer;
                    }
                ...
                {
                    if (P_field_description[P_pointer].element > 0)
                    {
                        changed = true;
                        --P_pointer;
                    }
            ...
            if (changed)
            {
                Display_old.P_swow_pointer_frame(P_field_description[P_pointer_old], false); // delete the old frame
                Display_old.P_swow_pointer_frame(P_field_description[P_pointer], true);      // show the new frame

                P_print_pointer_and_field_desciption(P_pointer);
            }
        }

        // Change P_pointer using Confirm
        if (P_field_description[P_pointer].field_name == field_Instrument && Read_pushbutton(EN_PB_Select))
        {
            P_pointer_old = P_pointer;
            ++P_pointer;
        ...
        else if (P_field_description[P_pointer].field_name == field_Instrument_inside && Read_pushbutton(EN_PB_Select))
        {
            P_pointer_old = P_pointer;
            P_pointer -= P_field_description[P_pointer].element + 1;
        ...
        case field_Menu:
        {
            if (Read_pushbutton(EN_PB_Select) || Read_pushbutton(EN_PB_Value))
            {
                ...
                case 0: // EXIT (drop Sound changes)
                    ...
                    Display_old.P_swow_pointer_frame(P_field_description[P_pointer], false); // delete the frame

                    patch_original = true;
                    P_Select_menu_elements();
                    P_Update_line_of_all_instruments();

                    Display_old.P_Performance_menu(); // displays the menu and updates "Value_Max_encoder.performance_menu" used by encoder_menu
                    Display_old.P_show_all_Instruments(Patch_id);

                    P_Update_field_description();
                    P_set_pointer_to_patch();
                    Display_old.P_swow_pointer_frame(P_field_description[P_pointer], true); // show the new frame
                ...
                case 4: // DROP Patch and go back to PERFORMANCE "P_Get_first_Patch_id_existing()"
                    ...
                    {
                        P_Select_menu_elements();
                        Display_old.P_Performance_page(false, true);

                        P_Update_field_description();
                        P_set_pointer_to_patch();
                        Display_old.P_swow_pointer_frame(P_field_description[P_pointer], true); // show the new frameP_pointer = 0;
                    }
                    break;
        ...
        case field_Instrument:
        {
            if (Read_pushbutton(EN_PB_Select) || Read_pushbutton(EN_PB_Value))
            {
                ...
                Instrument_id = P_instrument_id_from_pointer(P_pointer);
                Sound_id = P_sound_id_from_pointer(P_pointer);
                ...
                Noclick_max = Calc_Noclick_max(Preset[P_instrument_id_from_pointer(P_pointer)].use_Wavetable);
                trim_step = S_Calc_trim_step(trim_speed);
                sound_original = Verify_is_Sound_original(Sound_id);
                So_menu = 0;

                Display_old.Show_sound(Sound_id, P_instrument_id_from_pointer(P_pointer));

                // restore LEDs
                Performance_led_set.Restore_all_LED();

                Display_old.Show_wave(P_instrument_id_from_pointer(P_pointer));

                Select_sound_edit_menu_elements();
                Display_old.SOUND_EDIT_menu(); // displays the menu and updates "SO_menu_max" used by encoder_menuDisplay.Frame_SOUND_EDIT_menu
                Display_old.Frame_SOUND_EDIT_menu(So_menu);
        ...
        case field_Instrument_inside:
        {
            switch (P_field_description[P_pointer].element)
            {
            case 0: // Lock
                result = Read_encoder_simple(EN_PB_Value);
                if (result == 1)
                {
                    AudioNoInterrupts();
                    Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].lock = true;
                    Players_Manager.Update_Preset_lock(Patch_id, P_instrument_id_from_pointer(P_pointer));
                    Players_Manager.Multicast_reset_pitch_bend_effects(P_instrument_id_from_pointer(P_pointer));
                    AudioInterrupts();

                    // P_Macro_Instrument_editing(P_instrument_id_from_pointer(P_pointer));
                    P_Macro_Instrument_editing(Patch_id, P_instrument_id_from_pointer(P_pointer), P_field_description[P_pointer].element);
                    patch_original_0 = patch_original;
                    patch_original = P_Verify_is_Patch_original(Patch_id);
                    if (patch_original != patch_original_0)
                    {
                        P_Select_menu_elements();
                        Display_old.P_Performance_menu(); // display the menu and update "P_menu_max"
                        P_Update_field_description();
                    }
                }
                else if (result == -1)
                {
                    AudioNoInterrupts();
                    Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].lock = false;
                    Players_Manager.Update_Preset_lock(Patch_id, P_instrument_id_from_pointer(P_pointer));
                    Players_Manager.Broadcast_restore_pitch_bend_and_effects(P_instrument_id_from_pointer(P_pointer), pitch_bend_value[Get_midi_channel(Patch_id, P_instrument_id_from_pointer(P_pointer))]);
                    AudioInterrupts();
            ...
            case 1: // Precedence
                ...
                {
                    AudioNoInterrupts();
                    Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].precedence = true;
                    Players_Manager.Update_Preset_precedence(Patch_id, P_instrument_id_from_pointer(P_pointer));
                    AudioInterrupts();
                ...
                {
                    AudioNoInterrupts();
                    Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].precedence = false;
                    Players_Manager.Update_Preset_precedence(Patch_id, P_instrument_id_from_pointer(P_pointer));
                    AudioInterrupts();
            ...
            case 2: // Midi (channel)
                ...
                if (result != 0)
                {
                    ...
                    if (midi_channel_change != S_Get_midi_channel_from_Sound(Sound_id))
                    {
                        ...
                        P_Reset_map_Instrument_for_notes(P_instrument_id_from_pointer(P_pointer));
                        S_Set_midi_channel_for_Sound(Sound_id, midi_channel_change);
                        Update_map_Instrument_for_notes(Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].from_note, Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].to_note, P_instrument_id_from_pointer(P_pointer));
                        Players_Manager.Update_Preset_midi_channel(Patch_id, P_instrument_id_from_pointer(P_pointer));
                        AudioInterrupts();
            ...
            case 3: // Root key
                if (Read_encoder(EN_PB_Value, Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].root_key, 127, 0, 1))
                {
                    AudioNoInterrupts();
                    Players_Manager.Multicast_change_players_notes(Patch_id, P_instrument_id_from_pointer(P_pointer));
                    AudioInterrupts();
            ...
            case 4: // From Key
                ...
                {
                    changed = false;
                    if (result == 1 && Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].from_note < Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].to_note)
                    {
                        from_key_change = Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].from_note + 1;
                        changed = true;
                    }
                    else if (result == -1 && Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].from_note > 0)
                    {
                        from_key_change = Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].from_note - 1;
                        changed = true;
                    }
                    ...
                    {
                        AudioNoInterrupts();
                        Players_Manager.Change_from_key(Patch_id, P_instrument_id_from_pointer(P_pointer), from_key_change);
                        AudioInterrupts();
            ...
            case 5: // To key
                ...
                {
                    changed = false;
                    if (result == 1 && Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].to_note < 127)
                    {
                        to_key_change = Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].to_note + 1;
                        changed = true;
                    }

                    else if (result == -1 && Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].to_note > Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].from_note)
                    {
                        to_key_change = Patch[Patch_id].Instrument[P_instrument_id_from_pointer(P_pointer)].to_note - 1;
                        changed = true;
                    }
                    ...
                    {
                        AudioNoInterrupts();
                        Players_Manager.Change_to_key(Patch_id, P_instrument_id_from_pointer(P_pointer), to_key_change);
                        AudioInterrupts();
            ...
            case 6: // Pan
                if (Read_encoder(EN_PB_Value, Sound[P_sound_id_from_pointer(P_pointer)].pan, 16, -16, 1))
                {
                    AudioNoInterrupts();
                    Players_Manager.Update_Preset_pan(Patch_id, P_instrument_id_from_pointer(P_pointer));
                    Players_Manager.Multicast_pan(P_instrument_id_from_pointer(P_pointer));
                    AudioInterrupts();
                ...
                {
                    Sound[P_sound_id_from_pointer(P_pointer)].pan = 0;
            ...
            case 7: // Gain
                if (Read_encoder(EN_PB_Value, Sound[P_sound_id_from_pointer(P_pointer)].gain, 40, 0, 1))
                {
                    AudioNoInterrupts();
                    Players_Manager.Update_Preset_volume(Patch_id, P_instrument_id_from_pointer(P_pointer), Volume_float[volume_patch]);
                    Players_Manager.Multicast_volume_for_instrument_edit(P_instrument_id_from_pointer(P_pointer));
                    AudioInterrupts();
        ...
        {
            Display_old.P_show_Instrument_description(Patch_id, instrument_volume_changed, true);

            // restore LED
            if (Performance_led_set.Read_LED_activity(instrument_volume_changed) > 0)
            {
                Display_old.Led_PERFORMANCE_instrument(instrument_volume_changed, true);
            }
            else
            {
                Display_old.Led_PERFORMANCE_instrument(instrument_volume_changed, false);
            }
        ...
        if (!Read_pushbutton_fast(35))
        {
            if (Read_pushbutton(PB_number + 26) && Patch[Patch_id].Instrument[PB_number].used)
            {
                ...
                Display_old.Show_sound(Sound_id, Instrument_id);

                // restore LEDs
                Performance_led_set.Restore_all_LED();

                Display_old.Show_wave(Instrument_id);

                Select_sound_edit_menu_elements();
                Display_old.SOUND_EDIT_menu(); // displays the menu and updates "SO_menu_max" used by encoder_menu
                Display_old.Frame_SOUND_EDIT_menu(So_menu);
            }
...
#pragma region Sound Edit [rgba(2, 108, 118, 0.1)]
    ...
    if (Lilla_state == SOUND_EDIT)
    {
        ...
        if (Read_encoder(1, Sound[Sound_id].mode, 5, 0, 1))
        {
            ...
            Display_old.Play_mode(Instrument_id);
            Macro_Sound_menu();
        }
        ...
        if (result != 0)
        {
            ...
            if (file_midi_ch_flag)
            {
                ...
                if (S_file_change != Sound[Sound_id].file)
                {
                    ...
                    trim_step = S_Calc_trim_step(trim_speed);
                    So_menu = 0;
                    Display_old.File(Instrument_id);
                    Display_old.Show_wave(Instrument_id);
                    Macro_Sound_menu();
                }
            }
            ...
            else
            {
                ...
                if (midi_channel_change != S_Get_midi_channel_from_Sound(Sound_id))
                {
                    ...
                    Display_old.Midi_channel(Instrument_id);
                    Macro_Sound_menu();
                }
            }
        }
        ...
        if (Read_encoder(4, Sound[Sound_id].gain, 40, 0, 1))
        {
            ...
            Display_old.Gain_sound(Patch_id, Instrument_id);
            Display_old.Show_wave(Instrument_id);
            Macro_Sound_menu();
        }
        ...
        if (result != 0)
        {
            ...
            if (changed)
            {
                ...
                Display_old.Noclick(Instrument_id, true);
                Display_old.Show_wave(Instrument_id);
                Macro_Sound_menu();
            }
        }
        ...
        if (result != 0)
        {
            ...
            if (So_A_change != Sound[Sound_id].A)
            {
                ...
                Display_old.Pitch_voices_max(Instrument_id);
                Display_old.Show_wave(Instrument_id);
                Macro_Sound_menu();
            }
        }
        ...
        if (result != 0)
        {
            ...
            if (changed)
            {
                ...
                Display_old.Pitch(Instrument_id);
                Macro_Sound_menu();
            }
        }
        ...
        if (Read_encoder(18, Sound[Sound_id].attack, 255, 0, 1))
        {
            ...
            Display_old.Attack(Instrument_id);
            Macro_Sound_menu();
        }
        ...
        if (Read_encoder(19, Sound[Sound_id].decay, 255, 0, 1))
        {
            ...
            Display_old.Decay(Instrument_id);
            Macro_Sound_menu();
        }
        ...
        if (Read_encoder(20, Sound[Sound_id].sustain, 46, 4, 5))
        {
            ...
            Display_old.Sustain(Instrument_id);
            Macro_Sound_menu();
        }
        ...
        if (Read_encoder(21, Sound[Sound_id].release, 50, 0, 1))
        {
            ...
            Display_old.Release(Instrument_id);
            Macro_Sound_menu();
        }
        ...
        if ((Lilla_state_0 != MIDI_LOOP) && Read_pushbutton(25))
        {
            ...
            case 0: // Return - keep changes and exit from Sound EDITING
                ...
                Display_old.P_Patch_header(true, true); // P_Patch_header(bool change_patch, bool change_vol)
                P_pointer = P_menu_max + P_line_of_instrument[Instrument_id] + 1;
                P_Select_menu_elements();
                Display_old.P_Performance_menu(); // display the menu and update P_menu_max
                Display_old.P_show_Instruments_header();
                Display_old.P_show_all_Instruments(Patch_id);

                Display_old.P_show_Instrument_description(Patch_id, Instrument_id, true);

                // restore LED
                Performance_led_set.Restore_all_LED();

                Display_old.P_show_delete_Instrument_frame(P_line_of_instrument[Instrument_id], true);
            ...
            case 1: // Clone this Instrument and go back to PERFORMANCE
                ...
                AudioNoInterrupts();
                Instrument_id = S_Clone_Instrument(Instrument_id); // also increments instruments
                Sound_id = Patch[Patch_id].Instrument[Instrument_id].sound_id;
                Update_map_Instrument_for_notes(Patch[Patch_id].Instrument[Instrument_id].from_note, Patch[Patch_id].Instrument[Instrument_id].to_note, Instrument_id);
                Players_Manager.Update_Preset(Patch_id, Instrument_id, Volume_float[volume_patch]);
                Fill_Noclick(Instrument_id);
            ...
            case 2: // Delete this Instrument and go back to PERFORMANCE
                ...
                Display_old.P_Patch_header(true, false); // P_Patch_header(bool change_patch, bool change_vol)
                P_Select_menu_elements();
                Display_old.P_Performance_menu(); // displays the menu and updates P_menu_max used by encoder_menu
                P_pointer = P_menu_max + 1;
                Display_old.P_show_Instruments_header();
                Display_old.P_show_all_Instruments(Patch_id);
                Display_old.P_show_delete_Instrument_frame(P_line_of_instrument[Instrument_id], true);
                instrument_editing_flag = true;
        ...
        if (!Read_pushbutton_fast(35))
        {
            if (Read_pushbutton(PB_number + 26))
            {
                if (PB_number == Instrument_id)
                {
                    ...
                    Display_old.Instrument_VCF_page(Patch_id, Instrument_id);
    ...
    if (Lilla_state == INSTRUMENT_VCF)
    {
        if (Lilla_state_0 == LIVE_SAMPLING)
        {
            ...
            if (Read_encoder(15, volume_patch, 40, 0, 1))
            {
                ...
                Display_old.P_Patch_volume_value(true); // true: YELLOW
            }
        }
        ...
        else if (Lilla_state_0 == PERFORMANCE || Lilla_state_0 == MIDI_LOOP)
        {
            ...
            if (Read_encoder(4, Sound[Sound_id].gain, 40, 0, 1))
            {
                ...
                Display_old.Show_VCF_gain(Sound_id);
            }
        ...
        if (Read_encoder(5, Patch[Patch_id].Instrument[Instrument_id].Filter.type, 3, 0, 1))
        {
            ...
            Display_old.Show_VCF_filter_type(Instrument_id);
        }
        ...
        if (Read_encoder(6, Patch[Patch_id].Instrument[Instrument_id].Filter.modulation, 4, 0, 1))
        {
            ...
            Display_old.Show_VCF_lfo_type(Instrument_id);
        }
        ...
        if (Read_encoder(13, Patch[Patch_id].Instrument[Instrument_id].Filter.pivot, 100, 0, 1))
        {
            ...
            Display_old.Show_VCF_cutoff(Instrument_id);
        }
        ...
        if (Read_encoder(14, Patch[Patch_id].Instrument[Instrument_id].Filter.frequency_time, 40, 0, 1))
        {
            ...
            Display_old.Show_VCF_lfo_freq_time(Instrument_id);
        }
...

        {
            ...
            Display_old.Show_VCF_resonance(Instrument_id);
        }
...

        {
            ...
            Display_old.Show_VCF_lfo_index(Instrument_id);
        }
            ...
            if (Lilla_state_0 == PERFORMANCE)
            {
                if (Read_pushbutton(PB_number + 26))
                {
                    if (PB_number == Instrument_id)
                    {
                        ...
                        Display_old.P_Performance_page(true, true);
                        instrument_editing_flag = true;
                        P_pointer = P_menu_max + P_line_of_instrument[Instrument_id] + 1;
...
#pragma region Mixer [rgba(130, 4, 40, 0.19)]
    ...
    if (Lilla_state == MIXER)
    {
        ...
        if (!Read_pushbutton_fast(35))
        {
            if (Read_pushbutton(PB_number + 26) && Patch[Patch_id].Instrument[PB_number].used)
            {
                Display_old.MX_source_values_jump(MX_source, PB_number); // MX_source_values_jump(const uint8_t &old_source, const uint8_t &new_source) - qui si assegna il nuovo valore MX_source
            ...
            {
                // MX_source = 8;
                Display_old.MX_source_values_jump(MX_source, LINE_IN_CHANNEL); // aggiorna MX_source a 8
        ...
        if (MX_source < INSTRUMENTS_MAX)
        {
            if (Read_encoder(4, Sound[Sound_id].gain, 40, 0, 1))
            {
                ...
                Display_old.MX_source_values_edit(MX_source);
            }
        }
        ...
        if (Read_pushbutton_fast(35))
        {
            ...
            else if (Read_pushbutton(28))
            {
                ...
                {
                    Display_old.Delay_disabled();
                    delay(2000);

                    Display_old.MX_page();
                    for (auto source = 0; source < 9; ++source)
                    {
                        Display_old.MX_source_values(source);
                    }
                }
            }
...
#pragma region Delay [rgba(16, 86, 208, 0.25)]
    ...
    if (Lilla_state == DELAY_SETTINGS)
    {
        ...
        if (Read_encoder(20, Delay_data.modulation_depth, 39, 0, 1))
        {
            ...
            Display_old.D_modulation_depth();
        }
        ...
        if (Read_encoder(21, Delay_data.modulation_frequency, 90, 0, 1))
        {
            ...
            Display_old.D_modulation_frequency();
        }
        ...
        if (Read_encoder(22, Delay_data.modulation_phase_LR, 359, 0, 1))
        {
            ...
            Display_old.D_modulation_phase_LR();
        }
    ...
    if (Lilla_state == DIRECT_SAMPLING)
    {
        ...
        if (DS_state == 0 || DS_state == 1 || DS_state == 2)
        {
            ...
            if (PeakTracking_L.available())
            {
                ...
                Display_old.DS_bar(0, BAR_ELEMENTS + val); // Display_old.DS_bar(0, PeakTracking_L.read() * BAR_ELEMENTS);
            }
            if (PeakTracking_R.available())
            {
                val = 20 * log10(PeakTracking_R.read());
                Display_old.DS_bar(1, BAR_ELEMENTS + val); // Display_old.DS_bar(1, PeakTracking_R.read() * BAR_ELEMENTS);
            }
        }
        ...
        if (DS_state == 2)
        {
            ...
            if (DS_blink_timer >= 500)
            {
                ...
                Display_old.DS_sampler_txt(DS_blink_ON);
                DS_blink_timer = 0;
            }
            ...
            if (DS_recording_time_update >= 200)
            {
                ...
                Display_old.DS_update_recording_seconds(DS_recording_time);
                Display_old.DS_available_memory();
            }
            ...
            if (!DirectSampler.Is_recording())
            {
                ...
                DS_define_model();
                Display_old.DS_menu();
                Display_old.DS_frame_menu(DS_menu);
                Display_old.DS_available_memory();

                Display_old.DS_line_out(false);
                Display_old.DS_sampler_frame(true);
                Display_old.DS_sampler_txt(false);
        ...
        if (Read_pushbutton(25))
        {
            ...
            case 0: // Delete
                ...
                Display_old.DS_hide_recording();
                Display_old.DS_advice_delete(true);
            ...
            case 1: // Pause+Rec (pause before recording, listening Audio Input)
                ...
                Display_old.DS_update_volume(false); // cambia il colore del volume in bianco (fisso)
            ...
            case 2: // Mono Rec
                ...
                Display_old.DS_menu();
                Display_old.DS_frame_menu(DS_menu);
                Display_old.DS_Recording_description(recording, false);
                Display_old.DS_sampler_txt(true);
            ...
            case 5: // CONVERT_REC_TO_RAW
            {
                ...
                {
                    Display_old.DS_hide_recording();
                    Display_old.DS_advice_no_conversion(DS_export, true);
                    delay(7000);
                    Display_old.DS_advice_no_conversion(DS_export, false);
                    ...
                    Display_old.DS_Recording_description(recording, true);
                ...
                Display_old.DS_menu();
                Display_old.DS_frame_menu(DS_menu);
                Display_old.DS_conversion_options(file_L_RAW, file_R_RAW, DS_export);
                ...
                Display_old.DS_bar(0, 0);
                Display_old.DS_bar(1, 0);
            }
            break;
            ...
            case 11: // EXPORT AS RAW TO SD
            {
                ...
                {
                    DisplayPrimitives::Popup("SD CARD MISSING", ILI9341_WHITE, ILI9341_RED);
                    delay(2000);
                    DS_state = 0;
                    DS_menu = 0;
                    Display_old.DS_page(recording);
                    DS_define_model();
                    Display_old.DS_menu(); // display the menu and updates DS_menu_max
                    Display_old.DS_frame_menu(DS_menu);
                    // Switch bar_display ON
                    PeakTracking_L.reset();
                    PeakTracking_R.reset();
                ...
                if (!volume.init(card))
                {
                    DisplayPrimitives::Popup("SD CARD UNFORMATTED", ILI9341_WHITE, ILI9341_RED);
                    delay(2000);
                    DS_state = 0;
                    DS_menu = 0;
                    Display_old.DS_page(recording);
                ...
                else
                {
                    ...
                    if (DS_export == 0)
                    {
                        DisplayPrimitives::Popup("SD CARD IS FULL - CANNOT WRITE NEW FILES", ILI9341_WHITE, ILI9341_RED);
                        delay(2000);
                    ...
                    else
                    {
                        ...
                        if (!confirmation)
                        {
                            ...
                            DisplayPrimitives::Popup("/LILLARAW_EXPORT IS CROWDED --> DELETE SOME FILES", ILI9341_WHITE, ILI9341_RED);
                            delay(2000);
                        ...
                        else
                        {
                            ...
                            if (!Recording[recording].stereo)
                            {
                                DisplayPrimitives::Popup("MONO FILE EXPORTED TO SD", ILI9341_BLACK, ILI9341_GREEN);
                            }
                            else
                            {
                                DisplayPrimitives::Popup("LEFT AND RIGHT FILES EXPORTED TO SD", ILI9341_BLACK, ILI9341_GREEN);
                            }
                        }
                    }
    ...
    if (Lilla_state == MIDI_MONITOR)
    {
        ...
        if (display_wait)
        {
            ...
            case 0: // no message received
                break;
            case 1: // note ON
                Display_old.Midi_monitor_data(MM_midi_channel, 0, MM_note_number, MM_velocity, -1, -1);
                break;
            case 2: // note OFF
                Display_old.Midi_monitor_data(MM_midi_channel, 1, MM_note_number, MM_velocity, -1, -1);
                break;
            case 3: // pitch bend
                Display_old.Midi_monitor_data(MM_midi_channel, 2, -1, -1, (MM_pitch_bend_most << 7) + MM_pitch_bend_least, -1);
                break;
            case 4: // after touch poly
                Display_old.Midi_monitor_data(MM_midi_channel, 3, MM_least_bits, -1, MM_most_bits, -1);
                break;
            case 5: // control change
                Display_old.Midi_monitor_data(MM_midi_channel, 4, -1, -1, MM_midi_value, MM_midi_controller);
                break;
            case 6: // program change
                Display_old.Midi_monitor_data(MM_midi_channel, 5, -1, -1, -1, MM_least_bits);
                break;
            case 7: // After Touch Channel
                Display_old.Midi_monitor_data(MM_midi_channel, 6, -1, -1, MM_least_bits, -1);
                break;
            case 8: // System Exclusive
                Display_old.Midi_monitor_data(MM_midi_channel, 7, -1, -1, -1, -1);
                break;
            default:
    ...
    if (Lilla_state == MIDI_LOOP)
    {
        ...
        if (result != 0)
        {
            ...
            if (new_loop_id != -1 && new_loop_id != LOOP_id)
            {
                ...
                Display_old.Loop_loop_id();

                // update LOOP_time on display
                Display_old.Loop_time_stretched();

                // update tracks infos on display
                for (auto local_track = 0; local_track < TRACKS; ++local_track)
                {
                    Display_old.Loop_track_data(local_track);
                }

                // update menu on display
                LOOP_menu = 0;
                Display_old.Delete_all_frame_loop_menu();
                LOOP_select_menu_elements();
                Display_old.Loop_menu();
                Display_old.Frame_loop_menu(LOOP_menu, true);
        ...
        for (auto track = 0; track < TRACKS; ++track)
        {
            ...
            if (LOOP_run_button_state && Read_pushbutton(LOOP_UI_B + track))
            {
                ...
                if (LOOP_learning_track == MASTER_TRACK)
                {
                    ...
                    LOOP_time = 0;
                    Display_old.Loop_time_stretched(); // accanto ai led del metronomo appare il tempo totale 0.0s
                }
                ...
                if (LOOP_learning_track == MASTER_TRACK || (LOOP_learning_track > MASTER_TRACK && LOOP_events[MASTER_TRACK] != 0))
                {
                    ...
                    // display "n-REC"
                    Display_old.Loop_REC_advice(LOOP_learning_track, true);
                    ...
                    Display_old.Loop_track_data(track);
...
#pragma region[rgba(54, 135, 210, 0.2)]
    ...
    if (Lilla_state == SETUP)
    {
        ...
        {
            Display_old.Settings_frame(SET_menu);
        }
        ...
        if (Read_pushbutton(25))
        {
            ...
            case 3: // switch to CC Settings
                ...
                Display_old.CC_page();
                Display_old.All_CC_Sound_gain();
                Display_old.Show_CC_lowpass_filter();
                CC_menu = 0;
                Display_old.Frame_CC_page_menu(CC_menu);
                break;
            ...
            case 4: // import RAW files from SD
                ...
                {
                    Display_old.Settings_page();
                    Display_old.Settings_frame(SET_menu);
                }
                break;

            case 5: // Setup (all EEPROM content) import from lilla.txt (in SD)
                Display_old.Confirm_config_import_popup();
                Display_old.Confirm_config_import_frame(0);
                ...
                if (!SD.begin(BUILTIN_SDCARD))
                {
                    Display_old.SD_missing(ILI9341_BLACK);
                    delay(5000);
                    Display_old.Settings_page();
                    Display_old.Settings_frame(SET_menu);
                    break;
                ...
                {
                    Display_old.Config_import_FILE_error_popup();
                    delay(5000);
                    Display_old.Settings_page();
                    Display_old.Settings_frame(SET_menu);
                    break;
                ...
                {
                    Display_old.Config_import_REBOOT_popup();
            ...
            case 6: // Setup (all EEPROM content) export to SD card (lillaold.txt)
                Display_old.Confirm_config_export_popup();
                Display_old.Confirm_config_import_frame(0);
                Ask_if_IMPORT_EXPORT_setup();
                if (result == 0)
                {
                ...
                if (SD.begin(BUILTIN_SDCARD))
                {
                    ...
                    if (file)
                    {
                        ...
                        file.close();
                        Display_old.Config_export_save_popup();
                        delay(5000);
                    }
                    else
                    {
                        Display_old.Config_export_SD_error_popup();
                        delay(5000);
                    }
                    Display_old.Settings_page();
                    Display_old.Settings_frame(SET_menu);
            ...
            case 7: // Factory reset
                Display_old.Confirm_factory_reset_popup();
                Display_old.Confirm_config_import_frame(0);
    ...
    if (Lilla_state == CC_SETTINGS)
    {
        ...
        if (Read_encoder(24, CC_number, 127, 0, 1))
        {
            if (CC_menu > 0 && CC_menu < 9)
            {
                ...
                Display_old.Show_CC_Sound_gain(CC_menu - 1);
            }
            else if (CC_menu == 9)
            {
...
FLASHMEM
void P_Update_field_description(void)
{
    Serial.println("P_Update_field_description(void)");

    for (auto i = 0; i <= P_menu_max; ++i)
    {
        P_field_description[i].field_name = field_Menu;
        P_field_description[i].element = i;

        Serial.print(i);
        Serial.print("  field_Menu: ");
        Serial.println(P_field_description[i].element);
    }
    P_field_description[P_menu_max + 1].field_name = field_Patch;
    P_field_description[P_menu_max + 1].element = 0;

    Serial.print(P_menu_max + 1);
    Serial.print("  field_Patch: ");
    Serial.println(P_field_description[P_menu_max + 1].element);
    ...
    for (auto i = 0; i < (9 * (Patch[Patch_id].instruments)); ++i)
    {
        if ((i % 9) == 0)
        {
            P_field_description[place + i].field_name = field_Instrument;
            P_field_description[place + i].instrument_line = i / 9;
            P_field_description[place + i].instrument_id = instrument_on_position[i / 9];

            Serial.print(place + i);
            Serial.print("  field_Instrument - instrument_line: ");
            Serial.print(P_field_description[place + i].instrument_line);
            Serial.print("  instrument_id: ");
            Serial.println(P_field_description[place + i].instrument_id);
        }
        else
        {
            P_field_description[place + i].field_name = field_Instrument_inside;
            P_field_description[place + i].element = ((i % 9) - 1);
            P_field_description[place + i].instrument_line = i / 9;
            P_field_description[place + i].instrument_id = (instrument_on_position[i / 9]);

            Serial.print(place + i);
            Serial.print("  field_Instrument_inside - instrument_line: ");
            Serial.print(P_field_description[place + i].instrument_line);
            Serial.print("  instrument_id: ");
            Serial.print(P_field_description[place + i].instrument_id);
            Serial.print("  element: ");
            Serial.println(P_field_description[place + i].element);
        }
    }
}

void P_Restore_pointer(int P_menu_max_old, int P_menu_max)
{
    if (P_pointer > P_menu_max_old)
    {
        P_pointer += P_menu_max - P_menu_max_old;
    }
    else
    {
        P_pointer = 0;
    ...
    Serial.print("void P_Restore_pointer(int P_menu_max_old, int P_menu_max) - P_pointer: ");
    Serial.println(P_pointer);
    return;
}
...
int P_Ask_if_change_Patch(void)
{
    ...
    Display_old.P_Confirm_patch_change_popup();
    Display_old.P_Confirm_patch_change_popup_frame(0);
    delay(200);
    ...
    while (!confirmation)
    {
        ...
        {
            Display_old.P_Confirm_patch_change_popup_frame(action);
        }
...
bool P_Ask_if_delete_this_Patch(void)
{
    ...
    Display_old.P_Confirm_patch_delete_popup();
    Display_old.P_Confirm_patch_delete_popup_frame(0);
    delay(200);
    ...
    while (!confirmation)
    {
        ...
        {
            Display_old.P_Confirm_patch_delete_popup_frame(action);
        }
        if (Read_pushbutton(25))
        {
            confirmation = true;
...
void P_Macro_Instrument_editing(const int patch_id, const int instrument_id, const int element)
{
    ...
    {
    case 0: // Lock
        Display_old.P_show_Lock(patch_id, instrument_id, true);
        break;

    case 1: // Precedence
        Display_old.P_show_Precedence(patch_id, instrument_id, true);
        break;

    case 2: // Midi (channel)
        Display_old.P_show_Midi(patch_id, instrument_id, true);
        break;

    case 3: // Root key
        Display_old.P_show_RootKey(patch_id, instrument_id, true);
        break;

    case 4: // From Key
        Display_old.P_show_FromKey(patch_id, instrument_id, true);
        break;

    case 5: // To key
        Display_old.P_show_ToKey(patch_id, instrument_id, true);
        break;

    case 6: // Pan
        Display_old.P_show_Pan(patch_id, instrument_id, true);
        break;

    case 7: // Gain
        Display_old.P_show_Gain(patch_id, instrument_id, true);
        break;
    }
}
...
void P_Select_menu_elements(void)
{
    ...
    P_Restore_pointer(P_menu_max_old, P_menu_max);
}

int P_instrument_id_from_pointer(const int pointer)
{
    return P_field_description[pointer].instrument_id;
}

int P_sound_id_from_pointer(const int pointer)
{
    return Patch[Patch_id].Instrument[P_field_description[pointer].instrument_id].sound_id;
}
...
void P_print_pointer_and_field_desciption(const int pointer)
{
    Serial.print("P_pointer: ");
    Serial.println(pointer);

    Serial.print("P_field_description[P_pointer].field_name: ");
    Serial.print(P_field_description[pointer].field_name);
    Serial.print(" P_field_description[P_pointer].element: ");
    Serial.print(P_field_description[pointer].element);
    Serial.print(" P_field_description[P_pointer].instrument_line: ");
    Serial.print(P_field_description[pointer].instrument_line);
    Serial.print(" P_field_description[P_pointer].instrument_id: ");
    Serial.println(P_instrument_id_from_pointer(pointer));
}

void P_set_pointer_to_last_instrument(const int instrument_id)
{
    P_pointer = P_menu_max + 1 + P_line_of_instrument[instrument_id] * (P_Instrument_inside_elements + 1);
}
...
uint8_t S_Clone_Instrument(const int instrument_id)
{
    int new_instrument;
...
void P_Update_instruments_leds()
{
        ...
        for (auto track = 0; track < TRACKS; ++track)
        {
            for (auto instrument_id = 0; instrument_id < INSTRUMENTS_MAX; ++instrument_id)
            {
                if (Patch[Patch_id].Instrument[instrument_id].used) // check if i is used
                {
                    ...
                    {
                        Display_old.Loop_led(track, instrument_id, true);
                        Loop_led_set.Write_LED_activity(track, instrument_id, true);
                    }
                    if (Loop_led_set.Read_LED_activity(track, instrument_id) == -2)
                    {
                        Display_old.Loop_led(track, instrument_id, false);
    ...
    else if (Lilla_state == PERFORMANCE)
    {
        for (auto instrument_id = 0; instrument_id < INSTRUMENTS_MAX; ++instrument_id)
        {
            if (Patch[Patch_id].Instrument[instrument_id].used) // check if i is used
            {
                ...
                {
                    Display_old.Led_PERFORMANCE_instrument(instrument_id, true);
                    Performance_led_set.Write_LED_activity(instrument_id, true);
                }
                if (Performance_led_set.Read_LED_activity(instrument_id) == -2)
                {
                    Display_old.Led_PERFORMANCE_instrument(instrument_id, false);
        ...
        {
            TT_led_flag = false;
            Display_old.Led_tuning_tone(Patch_id);
        }
    }
    ...
    else if (Lilla_state == SOUND_EDIT)
    {
        ...
        if (Performance_led_set.Read_LED_activity(Instrument_id) == 2)
        {
            Display_old.Led_SOUND_EDIT_instrument(Instrument_id, true);
            Performance_led_set.Write_LED_activity(Instrument_id, true);
        }
        if (Performance_led_set.Read_LED_activity(Instrument_id) == -2)
        {
            Display_old.Led_SOUND_EDIT_instrument(Instrument_id, false);
            Performance_led_set.Write_LED_activity(Instrument_id, false);
        }
    ...
    else if (Lilla_state == INSTRUMENT_VCF)
    {
        ...
        if (Performance_led_set.Read_LED_activity(Instrument_id) == 2)
        {
            Display_old.Led_INSTRUMENT_VCF_instrument(Instrument_id, true);
            Performance_led_set.Write_LED_activity(Instrument_id, true);
        }
        if (Performance_led_set.Read_LED_activity(Instrument_id) == -2)
        {
            Display_old.Led_INSTRUMENT_VCF_instrument(Instrument_id, false);
            Performance_led_set.Write_LED_activity(Instrument_id, false);
        }
    ...
    else if (Lilla_state == DIRECT_SAMPLING)
    {
        ...
        {
            Display_old.Led_DIRECT_SAMPLING(true);
            if (Performance_led_set.Read_LED_activity(0) == 2)
            {
                Performance_led_set.Write_LED_activity(0, true);
            }
        ...
        {
            Display_old.Led_DIRECT_SAMPLING(false);
            Performance_led_set.Write_LED_activity(0, false);
...
void DS_ask_if_EXIT_from_DS(void)
{
    confirmation = false;
    action = 0; // NO
    Display_old.DS_confirm_EXIT_from_DS();
    Display_old.P_Confirm_patch_delete_popup_frame(0);
    delay(200);
...
void VFS_Make_VFS(void)
{
    Display_old.Make_VFS_presentation();
    ...
    if (VFS_packets_max > 40)
    {
        ...
        Display_old.Make_VFS_restart();
        delay(10000);
    }

    else
    {
        Display_old.Make_VFS_not_enough_memory_for_sampler();
        delay(10000);
    }
}
...
bool Verify_is_Sound_original(const int sound_id)
{
    return (
...
void Ask_if_IMPORT_EXPORT_setup(void)
{
    ...
    while (!confirmation)
    {
        ...
        {
            Display_old.Confirm_config_import_frame(result);
        }
        if (Read_pushbutton(25))
        {
            confirmation = true;
...
bool Copy_raw_files_from_SD_to_Flash()
{
    ...
    Display_old.Copy_raw_files_SD_to_Flash_chip_titolo();

    // Wait for SD card
    while (!SD.begin(BUILTIN_SDCARD))
    {
        Display_old.Copy_raw_files_SD_to_Flash_chip_waiting_for_SD();
        delay(10000);
        return false;
    }
    DisplayPrimitives::Delete_row(3);

    // Check if LILLARAW directory exists
    if (!SD.exists("/LILLARAW"))
    {
        Display_old.Copy_raw_files_SD_to_Flash_chip_lillaraw_missing();
        delay(4000);
        return false;
    }
    ...
    Display_old.Copy_raw_files_SD_to_Flash_chip_files_report(SD_raw_volume, SD_raw_files, Get_raw_files_volume(), Get_raw_files());
    ...
    */
    ...
    Display_old.Copy_raw_files_SD_to_Flash_chip_last_warning(erasing_time_ms);

    // Confirmation
    bool confirm = false;
    uint8_t action = 0;
    Display_old.Import_raw_files_frame(action);
    ...
    Display_old.Copy_raw_files_SD_to_Flash_chip_job_start();
    ...
    Display_old.Copy_raw_files_SD_to_Flash_chip_initial_percentage();

    while (SerialFlash.ready() == false)
    {
        if (dotMillis > erasing_time_ms_step)
        {
            auto x_coordinate = DisplayPrimitives::display_coordinate_x(0);
            DisplayPrimitives::Cancel_text(x_coordinate + barcount + 10, BAR_POS_Y, 3);
            dotMillis = 0;
            barcount = barcount + 1;
            if (barcount <= 100)
            {
                Display_old.Copy_raw_files_SD_to_Flash_chip_progress(barcount);
            }
        }
    }

    // Start copying RAW files from SD to Flash chip
    Display_old.Copy_raw_files_SD_to_Flash_chip_popup_landscape();
    ...
    while (1)
    {
        ...
        Display_old.Copy_raw_files_SD_to_Flash_chip_files_to_copy(row, filename, length);
        ...
        if (SerialFlash.create(filename, length))
        {
            ...
            {
                Display_old.Copy_raw_files_SD_to_Flash_chip_flash_error();
            }
        }

        else
        {
            Display_old.Copy_raw_files_SD_to_Flash_chip_flash_full_error();
        }
        f.close();
    }
    rootdir.close();
    ...
    Display_old.Copy_raw_files_SD_to_Flash_chip_job_done();
    row = 2;
    SerialFlash.opendir();
    ...
    while (SerialFlash.readdir(filename, sizeof(filename), filesize))
    {
        ...
        {
            delay(4000);
            Display_old.Copy_raw_files_SD_to_Flash_chip_list_landscape();
            row = 3;
        }

        Display_old.Copy_raw_files_SD_to_Flash_chip_file_copied(row, filename, filesize);
    }
    delay(6000);
    return true;
}
...
void Macro_Sound_menu(void)
{
    ...
    if (sound_original_0 != sound_original)
    {
        ...
        Display_old.SOUND_EDIT_menu(); // displays the menu and updates "SO_menu_max" used by encoder_menu
        Display_old.Frame_SOUND_EDIT_menu(0);
    }
}
...
void Bootstrap_setup(void)
{
    ...
    // *******************    COVER PAGE    **********************
    Display_old.Lilla_cover_slow();
    // Display_old.Lilla_cover_saturate();
