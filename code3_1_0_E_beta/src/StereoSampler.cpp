...
bool StereoSampler::Start(int from_packet, int last_packet, int recording_id_in, bool stereo_in)
{
    ...
    if (stereo_flag)
    {
        ...
        if ((last_packet - from_packet + 1) % 2 == 0)
            ...
            last_packet_L = last_packet - 2; // 13
...
void StereoSampler::update(void)
{
    ...
    // Serial.print(F("SSampler - I see :"));
    // Serial.print(_AudioQueue_L->available());
    // Serial.print("   blocks in AudioQueue_L, and: ");
    // Serial.print(_AudioQueue_R->available());
    // Serial.println("   blocks in AudioQueue_R");
    ...
    if (stereo_flag)
    {
        ...
        else if (attack_gain_flag)
        {
            ...
            {
                *(_RAM_buffer_L + i) = (float)*(_RAM_buffer_L + i) * (1.0 - samples_counter / F_Slope_samples);
                *(_RAM_buffer_R + i) = (float)*(_RAM_buffer_R + i) * (1.0 - samples_counter / F_Slope_samples);
                samples_counter--;
            }
        ...
        else if (decay_gain_flag)
        {
            ...
            for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++)
            {
                *(_RAM_buffer_L + i) = (float)*(_RAM_buffer_L + i) * (samples_counter / F_Slope_samples);
                *(_RAM_buffer_R + i) = (float)*(_RAM_buffer_R + i) * (samples_counter / F_Slope_samples);
                samples_counter--;
            }
            Packet_L.write(RAM_buffer_L, 256);
            Packet_R.write(RAM_buffer_R, 256);
    ...
    else
    {
        if (!attack_gain_flag && !decay_gain_flag)
        {
            ...
            for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++)
            {
                *(_RAM_buffer_L + i) = (*(_RAM_buffer_L + i) + *(_RAM_buffer_R + i)) / 2;
            }
        ...
        else if (attack_gain_flag)
        {
            ...
            for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++)
            {
                *(_RAM_buffer_L + i) = (1.0 - samples_counter / F_Slope_samples) * (float)(*(_RAM_buffer_L + i) + *(_RAM_buffer_R + i)) / 2.0;
                samples_counter--;
            }
        ...
        else if (decay_gain_flag)
        {
            ...
            for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++)
            {
                *(_RAM_buffer_L + i) = (samples_counter / F_Slope_samples) * (float)(*(_RAM_buffer_L + i) + *(_RAM_buffer_R + i)) / 2.0;
                samples_counter--;
            }
            // copy block from RAM_buffer_L to Packet_L
            Packet_L.write(RAM_buffer_L, 256);
