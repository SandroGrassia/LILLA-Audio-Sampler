...
bool NoclickCrossmix::Make(int file_id, int32_t A_Flash_sample, int32_t B_Flash_sample, uint16_t delta_Noclick)
{
    ...
    for (uint16_t i = 0; i < delta_Noclick; i++)
    {
        float h = (float)i / ((float)delta_Noclick - 1.0); // h: 0 --> 1.0
        cache[i] = (float)cache[i] * h;
        Noclick[i] = ((float)Noclick[i] * (1.0 - h)) + cache[i];
        // Serial.print(i);
        // Serial.print(" - ");
        // Serial.println(Noclick[i]);
    }
    return true;
}
