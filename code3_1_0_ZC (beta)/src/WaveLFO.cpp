...
void WaveLFO::Update(void)
{
    if (periodic == 0) // aperiodic wave
    {
        for (auto sample = 0; sample < AUDIO_BLOCK_SAMPLES; ++sample)
        {
            if (wave_end)
            {
                block[sample] = local_waveform[10];
            }

            else
            {
                int I_wave_point = floor(wave_point);
                block[sample] = local_waveform[I_wave_point] + (local_m_waveform[I_wave_point] * (wave_point - I_wave_point));
    ...
    else if (periodic == 1) // periodic wave (sinus)
    {
        ...
        for (auto sample = 0; sample < AUDIO_BLOCK_SAMPLES; ++sample)
        {
            ...
            val2 *= scale;
            val1 *= 0x10000 - scale;
            block[sample] = multiply_32x32_rshift32(val1 + val2, magnitude); // ((((val1 + val2) >> 16) * LFO_magnitude) >> 16) ;
            phase += inc;
        }
