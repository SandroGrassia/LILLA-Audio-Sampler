...
void AudioPlayer::Main_settings(uint8_t mode_in, int A_value_in, int B_value_in, uint16_t delta_Noclick_in, bool use_Wavetable_in, int16_t *p_Noclick_in, int16_t *p_Wavetable_in)
{
    ...
    if (file_id_wait >= FIRST_LIVE_SAMPLING_FILE)
    {
        ...
        {
            LS_Q_sample = _LiveSampler->Q_sample;
...
void AudioPlayer::Start_playing(void)
{
    ...
    else if (file_id < FIRST_LIVE_SAMPLING_FILE)
    {
        ...
        {
            if ((file_id - FIRST_RECORDING_FILE) % 2 == 0)
                first_packet = Recording[recording].first_packet; // file LEFT
            else
                first_packet = Recording[recording].first_packet + 1; // file RIGHT
        }
    ...
    {
        _Players_statistics->Inc_total_Players_per_track_instrument(track, instrument);
        Set_led_flags(track);
    }

    else if (Lilla_state != MIDI_LOOP)
    {
        _Players_statistics->Inc_total_Players_per_instrument(instrument);
        Set_led_flags(track);
    }
}
...
void AudioPlayer::Release_note(void) // release note, fires ADSR "release"
{
    ...
    if (Lilla_state == MIDI_LOOP && track >= 0)
    {
        _Players_statistics->Dec_total_Players_per_track_instrument(track, instrument);
        Set_led_flags(track);
    }

    else if (Lilla_state != MIDI_LOOP)
    {
        _Players_statistics->Dec_total_Players_per_instrument(instrument);
        Set_led_flags(track);
    }
}
...
void AudioPlayer::Connect_VCF(bool use, int type, float pivot, float resonance, bool modulated)
{
    ...
    {
        _VCF->LFO_connect = true;
        _VCF->filter_type = type;
        _VCF->q_value = resonance; // Butterworth: 0.7071

        _VCF->_block = block;                   // int16_t*
        _VCF->_frequency = VCF_frequency_array; // float*
    }

    else if (VCF_connect && !VCF_modulated)
    {
        _VCF->LFO_connect = false;
        _VCF->filter_type = type;
        _VCF->q_value = resonance;                // Butterworth: 0.7071
        _VCF->Set_filter(0, VCF_frequency_pivot); // Set_filter(uint32_t stage, float frequency, float q = 0.7071)
...
void AudioPlayer::Connect_LFO_TO_VCF(uint8_t modulation, float index, uint8_t periodic, float frequency_time) // used by Midi_Reader
{
    ...
    if (LFO_periodic == 1)
    {
        _LFO_P0->Set_frequency(frequency_time);
    }
    else
    {
    ...
    if (VCF_connect)
    {
        ...
        if (VCF_modulated)
        {
            if (LFO_periodic == 1)
            {
                ...
                _LFO_P0->Set_amplitude(3000); // amplitude <= 32767
            }
            else
            {
                // Serial.println("LFO Setup: APERIODIC");
                _LFO_P0->Setup_aperiodic_wave(LFO_seconds, LFO_index * 3000, LFO_modulation); // _LFO_P0->SETUP_wave(LFO_seconds, LFO_index * 1000.0 * pitch, LFO_modulation); // SETUP_wave(float seconds, float max_value, int waveform) // aperiodic wave - period: seconds  n: waveform
            }
        }
    }
}
...
void AudioPlayer::Send_LFO_to_VCF(void)
{
    ...
    {
        _VCF->Set_filter(0, constrain(VCF_central_frequency * pow(2.0f, (_LFO_P0->block[0] / 1000.0f) * LFO_index), 50, 15000));     // Set_filter(uint32_t stage, float frequency)
        VCF_frequency_array[0] = constrain(VCF_central_frequency * pow(2.0f, (_LFO_P0->block[0] / 1000.0f) * LFO_index), 50, 15000); // ONLY FOR PRINT
        VCF_frequency_array[1] = constrain(VCF_central_frequency * pow(2.0f, (_LFO_P0->block[ABS_4] / 1000.0f) * LFO_index), 50, 15000);
        VCF_frequency_array[2] = constrain(VCF_central_frequency * pow(2.0f, (_LFO_P0->block[ABS_2] / 1000.0f) * LFO_index), 50, 15000);
        VCF_frequency_array[3] = constrain(VCF_central_frequency * pow(2.0f, (_LFO_P0->block[ABS_3_4] / 1000.0f) * LFO_index), 50, 15000);
    }
    ...
    else if (VCF_pivot_steps > 0)
    {
        ...
        VCF_central_frequency = (VCF_frequency_pivot * pitch);
        _VCF->Set_filter(0, constrain(VCF_central_frequency * pow(2.0f, (_LFO_P0->block[0] / 1000.0f) * LFO_index), 50, 15000));     // Set_filter(uint32_t stage, float frequency, float q = 0.7071)
        VCF_frequency_array[0] = constrain(VCF_central_frequency * pow(2.0f, (_LFO_P0->block[0] / 1000.0f) * LFO_index), 50, 15000); // ONLY FOR PRINT
        VCF_frequency_pivot += VCF_pivot_grain;
        VCF_central_frequency = (VCF_frequency_pivot * pitch);
        VCF_frequency_array[1] = constrain(VCF_central_frequency * pow(2.0f, (_LFO_P0->block[ABS_4] / 1000.0f) * LFO_index), 50, 15000);
