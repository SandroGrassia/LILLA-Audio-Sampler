...
// **************      VERSIONE FIRMWARE       **************
// **********************************************************
String FIRMWARE_VERSION = "3.1.0_beta 03/06/2025";
...
AudioGain D_gain_L_n;
Mixer_2x1 D_mixer_L_in;
StereoDelay Delay_L;
...
AudioConnection patchCord33(Router_L, 0, D_mixer_L_in, 0);
AudioConnection patchCord34(D_mixer_L_in, 0, Delay_L, 0);
AudioConnection patchCord35(D_mixer_L_in, 0, mixer_L, 0);
AudioConnection patchCord36(Delay_L, 0, D_gain_L_n, 0);
AudioConnection patchCord37(D_gain_L_n, 0, D_mixer_L_in, 1);
...
PlayersManager Players_Manager(&Player[0], &LOOP_led_set, &Players_statistics, &Router_L, &Router_R, &Noclick[0], &Wavetable[0]);
...
void LS_ask_if_exit_from_LS(void);
...
void setup()
{
    ...
    */
    ...
    for (uint8_t p = 0; p < PLAYERS; p++)
    {
        ...
        Player[p]._VCF = &VCF[p];
        Player[p]._LFO_P0 = &LFO_P0[p];
        Player[p]._LiveSampler = &LiveSampler;
        Player[p]._Players_statistics = &Players_statistics;
        Player[p]._LOOP_led_set = &LOOP_led_set;
        LFO_P0[p].identity = p;
    }
    ...
    // Setup Midi_reader
    Midi_reader._Vibrato = &Vibrato;
    Midi_reader._Tone_generator = &Tone_generator;
    Midi_reader._LOOP_led_set = &LOOP_led_set;
    Midi_reader._Players_Manager = &Players_Manager;
    Midi_reader._Players_statistics = &Players_statistics;

    // Setup Execute_Commands
    Filter_Biquad_Manager._biquad_L = &biquad_L;
    Filter_Biquad_Manager._biquad_R = &biquad_R;

    // Setup Trigger
    Trigger_0.identity = 0;
    Trigger_1.identity = 1;
    Trigger_0._Midi_reader = &Midi_reader;
    Trigger_1._Midi_reader = &Midi_reader;
    Trigger_0._Filter_Biquad_Manager = &Filter_Biquad_Manager;
    Trigger_1._Filter_Biquad_Manager = &Filter_Biquad_Manager;
    Trigger_0._Delay_Manager = &Delay_manager;

    // Setup Delays
    Delay_L._LFO = &LFO_D[0];
    Delay_R._LFO = &LFO_D[1];
    LFO_D[0].identity = 88;
    LFO_D[0].identity = 99;

    // Setup Infotest
    Info._LiveSampler = &LiveSampler;

    // Setup Wavetable-s
    for (uint8_t i = 0; i < INSTRUMENTS_MAX; ++i)
        Wavetable[i]._LiveSampler = &LiveSampler;
    ...
    // Setup LOOP_led_set
    LOOP_led_set._Players_statistics = &Players_statistics;

    // DelayManager
    Delay_manager._Delay_L = &Delay_L;
    Delay_manager._Delay_R = &Delay_R;
    Delay_manager._LFO_D[0] = &LFO_D[0];
    Delay_manager._LFO_D[1] = &LFO_D[1];
    Delay_manager._D_gain_L_n = &D_gain_L_n;
    Delay_manager._D_gain_R_n = &D_gain_R_n;
    Delay_manager._Players_Manager = &Players_Manager;
    ...
    if (Lilla_state == SOUND_EDIT)
    {
        ...
        if (result != 0)
        {
            ...
            if (So_A_change != Sound[id_sound].A)
            {
                ...
                Players_Manager.Verify_if_stop_players(instrument);
        ...
        if (result != 0)
        {
            ...
            if (So_B_change != Sound[id_sound].B)
            {
                ...
                // verify if stop players: it can happend if use_Wavetable switches to "false". Than update Preset[I].A (DO NOT invert the sequence)
                Players_Manager.Verify_if_stop_players(instrument); // MUST be done BEFORE updating Preset
                Players_Manager.Update_Preset_A_B_Wavetable(session, instrument);
                Noclick_max = Calc_Noclick_max(Preset[instrument].use_Wavetable);
                if (Sound[id_sound].Noclick > Noclick_max)
                {
...
#pragma region Mixer [rgba(130, 4, 40, 0.19)]
    ...
    if (Lilla_state == MIXER)
    {
        ...
        {
            if (MX_routing_source[MX_source] == 0)      // era tutto muto
                MX_routing_source[MX_source] = 2;       // --> solo MAIN
            else if (MX_routing_source[MX_source] == 1) // era solo MONITOR
                MX_routing_source[MX_source] = 3;       // --> MONITOR e MAIN
            else if (MX_routing_source[MX_source] == 2) // era solo MAIN
                MX_routing_source[MX_source] = 0;       // --> tutto muto
            else                                        // era 3 (MONITOR e MAIN)
                MX_routing_source[MX_source] = 1;       // --> solo MONITOR
    ...
    if (Lilla_state == DELAY_SETTINGS)
    {
        ...
        if (Read_encoder(13, Delay_data.samples, 99, 0, 1))
        {
            ...
            if (Delay_values.samples_LR >= 0)
                Delay_L.Set_delay_central_value(Delay_values.samples + Delay_values.samples_LR); // Left channel
            else
                Delay_R.Set_delay_central_value(Delay_values.samples - Delay_values.samples_LR); // Left channel
            AudioInterrupts();
...
// change MENU item
        ...
        if (Read_pushbutton(25))
        {
            ...
            case 3: // Erase
                ...
                if (LS_stereo)
                {
                    LS_erase_FIFO_array(_LS_buffer_L, LS_stereo); // LS_erase_FIFO_array(const int16_t* Array, int stereo)
                    LS_erase_FIFO_array(_LS_buffer_R, LS_stereo); // LS_erase_FIFO_array(const int16_t* Array, int stereo)
                }
                else
                    LS_erase_FIFO_array(_LS_buffer_mono, LS_stereo); // LS_erase_FIFO_array(const int16_t* Array, int stereo)
...
void Switch_from_LIVE_SAMPLING_to_DIRECT_SAMPLING(void)
{
    if (LS_state == 1)
    {
        LS_ask_if_exit_from_LS();
        if (action == 0) // false: remain
        {
            if (Lilla_state == LIVE_SAMPLING)
                LS_refresh_LS_page();
            else if (Lilla_state == INSTRUMENT_VCF)
...
void Fill_all_Noclick(void)
{
    for (int instrument = 0; instrument < INSTRUMENTS_MAX; ++instrument)
        if (Session[session].Instrument[instrument].used)
            Noclick[instrument].Make(Preset[instrument].file, Preset[instrument].A, Preset[instrument].B, Preset[instrument].Noclick); // Fill_Noclick(instrument);
}
...
uint8_t Encoder_state(uint8_t encoder, bool write_mux)
{
#define PAUSE_MUX 10 // microseconds between send address and read value
    if (write_mux)
    {
...
bool Read_pushbutton(uint8_t PB_id)
{
#define PB_UP_PAUSE 50
#define PB_DOWN_PAUSE 50
...
void LS_Setup_buffers(bool stereo, bool first)
{
    ...
    *** LS_Setup_buffers - indirizzo _LS_buffer_L tx: 700D7844
    decimale: 1879930948
    indirizzo _LS_buffer_R tx: 7086BA64
    decimale: 1887877732
    ...
    */
    ...
    if (stereo)
    {
        ...
        if (!first)
            PSRAM_Manager.Remove_samples_array(_LS_buffer_mono);

        // Inizializza PSRAM
        _LS_buffer_L = PSRAM_Manager.New_samples_array(LS_STEREO_BYTES);
        _LS_buffer_R = PSRAM_Manager.New_samples_array(LS_STEREO_BYTES);

        // Aggiorna oggetti
        LiveSampler._LS_buffer_L = _LS_buffer_L;
        LiveSampler._LS_buffer_R = _LS_buffer_R;
        Info._LS_buffer_L = _LS_buffer_L;
        Info._LS_buffer_R = _LS_buffer_R;

        Players_Manager.Broadcast_FIFO_stereo(_LS_buffer_L, _LS_buffer_R);

        Serial.print("*** LS_Setup_buffers - indirizzo _LS_buffer_L tx: ");
        Serial.println((unsigned long)_LS_buffer_L, HEX);
        Serial.print("decimale: ");
        Serial.println((unsigned long)_LS_buffer_L);
        Serial.print("indirizzo _LS_buffer_R tx: ");
        Serial.println((unsigned long)_LS_buffer_R, HEX);
        Serial.print("decimale: ");
        Serial.println((unsigned long)_LS_buffer_R);
    }
    ...
    else
    {
        ...
        {
            // Libera la PSRAM
            PSRAM_Manager.Remove_samples_array(_LS_buffer_L);
            PSRAM_Manager.Remove_samples_array(_LS_buffer_R);
        }

        // Inizializza PSRAM
        _LS_buffer_mono = PSRAM_Manager.New_samples_array(LS_MONO_BYTES);

        // Aggiorna oggetti
        LiveSampler._LS_buffer_mono = _LS_buffer_mono;
        Info._LS_buffer_mono = _LS_buffer_mono;
        Players_Manager.Broadcast_FIFO_mono(_LS_buffer_mono);

        Serial.print("*** LS_Setup_buffers - indirizzo _LS_buffer_mono: ");
        Serial.println((unsigned long)_LS_buffer_mono, HEX);
        Serial.print("decimale: ");
        Serial.println((unsigned long)_LS_buffer_mono);
    }
}
...
float eraseBytesPerSecond(const unsigned char *id)
{
    ...
    if (id[0] == 0xEF)
        return 512281.4; // 419430.0 Winbond
    if (id[0] == 0xC2)
